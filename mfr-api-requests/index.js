...
function setupApp () {
  ...
  app.all('*', async (req, res) => {
    ...
    ********************/
    ...
    try {
      responseBody = await fs.readFileSync(__dirname + req.url, 'utf8')
    } catch (err) {
      ...
      var properties = {}
      res.send(utils.buildReturnObject(mediatorConfig.urn, 'Failed', 404, headers, responseBody, orchestrations, properties))
      return
    }
    /********************
      DEMO PURPOSE ONLY
    ********************/
    ...
    // Send a GET request to the DHIS2 instance
    // let response = await fetch(config.dhisServer + req.url)
